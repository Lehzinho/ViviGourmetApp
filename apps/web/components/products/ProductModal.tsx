...
import type { CreateProductPayload, ExtraCostDraft } from "@/types/products";
import {
  Field,
  Input,
  Label,
...
export function ProductModal({
  ...
  const handleSubmit = () => {
    ...
    onSubmit({
      ...
      photoUrl: meta.photoUrl.trim() || undefined,
      weight: meta.weight ? parseDecimalInput(meta.weight) : undefined,
      dimensions: meta.dimensions.trim() || undefined,
      items: lines
  ...
  return (
    ...
    >
      <Panel onClick={(e) => e.stopPropagation()}>
        <ModalHead>
          <ModalTitle id="product-modal-title">Novo produto</ModalTitle>
              ...
              <MetaGrid>
                <Field>
                  ...
                  <Input
                    ...
                    placeholder="Ex.: Biscoito de manteiga"
                  />
                </Field>
                <Field>
                  <Label htmlFor="pm-desc">Descrição</Label>
                  <Input
                    id="pm-desc"
                ...
                <Field>
                  <Label htmlFor="pm-photo">URL da foto</Label>
                  <Input
                    id="pm-photo"
                    value={meta.photoUrl}
                    onChange={(e) => setMeta((m) => ({ ...m, photoUrl: e.target.value }))}
                    placeholder="https://…"
                  />
                </Field>
                <Field>
                  <Label htmlFor="pm-weight">Peso (g)</Label>
                  <Input
                    id="pm-weight"
                    type="text"
                    inputMode="decimal"
                    value={meta.weight}
                    onChange={(e) => setMeta((m) => ({ ...m, weight: e.target.value }))}
                    placeholder="Ex.: 50"
                  />
                </Field>
                <Field>
                  <Label htmlFor="pm-dimensions">Dimensões</Label>
                  <Input
                    id="pm-dimensions"
                    value={meta.dimensions}
                    onChange={(e) => setMeta((m) => ({ ...m, dimensions: e.target.value }))}
                      ...
                      <tr key={line.id}>
                        ...
                        <Td>
                          <Select
                            value={line.unit}
                            onChange={(e) =>
                              updateLine(line.id, { unit: e.target.value as IngredientUnit })
                            }
                            style={{ width: "72px" }}
                          >
                            {unitOptions.map((o) => (
                  ...
                  {extraCosts.length === 0 ? (
                    <tr>
                      <Td colSpan={3} style={{ color: "#71717a", fontStyle: "italic" }}>
                        Nenhum custo extra. Ex.: embalagem, mão de obra.
                      </Td>
                    </tr>
                  ) : (
                    extraCosts.map((ec) => (
                      <tr key={ec.id}>
                        ...
                        <Td>
                          <IconBtn
                            ...
                            aria-label="Remover"
                            onClick={() =>
                              setExtraCosts((prev) => prev.filter((e) => e.id !== ec.id))
                            }
                          >
                            ✕
                          </IconBtn>
